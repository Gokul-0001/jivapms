...
from app_backlog.mod_backlog_type.forms_backlog_type import *
...
# ============================================================= #
@login_required
def list_backlog_types(request, org_id, parent_id):
    # take inputs
        ...
        if 'selected_item' in request.POST:  
            ...
            for item_id in selected_items:
                ...
                if bulk_operation == 'bulk_delete':
                    ...
                    return redirect('list_backlog_types', parent_id=ref_parent_id)
            return redirect('list_backlog_types', parent_id=ref_parent_id)
...
# ============================================================= #
@login_required
def list_deleted_backlog_types(request, org_id, parent_id):
    # take inputs
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
        ...
        if 'selected_item' in request.POST:  
                ...
                for item_id in selected_items:
                    ...
                    if bulk_operation == 'bulk_restore':
                        ...
                        return redirect('list_deleted_backlog_types', parent_id=ref_parent_id)
                return redirect('list_deleted_backlog_types', parent_id=ref_parent_id)
...
# Create View
@login_required
def create_backlog_type(request, org_id,parent_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
    ...
    if request.method == 'POST':
        ...
        return redirect('list_backlog_types', org_id=org_id, parent_id=ref_parent_id)
    else:
...
# Edit
@login_required
def edit_backlog_type(request, org_id,parent_id,  content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
...
@login_required
def delete_backlog_type(request, org_id, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
...
@login_required
def permanent_deletion_backlog_type(request, org_id, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
...
@login_required
def restore_backlog_type(request, org_id, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
    ...
    object = get_object_or_404(BacklogType, org_id=org_id,  pk=content_type_id, active=False, **viewable_dict)
    object.active = True
    object.save()
    return redirect('list_backlog_types', org_id=org_id, parent_id=ref_parent_id)
...
@login_required
def view_backlog_type(request, org_id, parent_id,  content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
     ...
     # connect with connect id
    org = get_object_or_404(Organization, pk=org_id, **viewable_dict)
    object = get_object_or_404(BacklogType, org_id = org_id, pk=content_type_id, active=True, **viewable_dict)    
...
@login_required
def copy_backlog_type(request, org_id , parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
...
@login_required
def view_backlog_type_tree(request, org_id, parent_id):
    # take inputs
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
