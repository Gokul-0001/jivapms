...
def list___pluralmodname__(request, __connectid__, parent_id):
    ...
    if search_query:
        ...
        deleted = __modelname__.objects.filter(active=False, deleted=False,
                                               __connectid__=__connectid__,
    ...
    context = {
        ...
        'parent_id': parent_id,  
        '__connectstr__': __connectstr__,  # '___connectstr___'
        '__connectid__': __connectid__,  # '___connectid___'
...
def list_deleted___pluralmodname__(request, __connectid__, parent_id):
    ...
    if search_query:
        ...
        tobjects = __modelname__.objects.filter(parent_id=parent_id, active=False, 
                                                __connectid__=__connectid__,
...
# Edit
@login_required
def edit___singularmodname__(request, __connectid__,parent_id,  content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
     ...
     # connect with connect id
    __connectstr__ = get_object_or_404(__connectmoduletitle__, pk=__connectid__, **viewable_dict)
    object = get_object_or_404(__modelname__, pk=content_type_id, active=True, **viewable_dict)
    if request.method == 'POST':
...
@login_required
def delete___singularmodname__(request, __connectid__, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
...
@login_required
def permanent_deletion___singularmodname__(request, __connectid__, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
     ...
     # connect with connect id
    __connectstr__ = get_object_or_404(__connectmoduletitle__, pk=__connectid__, **viewable_dict)
    object = get_object_or_404(__modelname__, pk=content_type_id, active=False, deleted=False, **viewable_dict)
    if request.method == 'POST':
        object.active = False
        object.deleted = True
        object.save()
...
@login_required
def restore___singularmodname__(request, __connectid__, parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
    ...
    object = get_object_or_404(__modelname__, __connectid__=__connectid__,  pk=content_type_id, active=False, **viewable_dict)
    object.active = True
    object.save()
    return redirect('list___pluralmodname__', __connectid__=__connectid__, parent_id=ref_parent_id)
...
@login_required
def view___singularmodname__(request, __connectid__, parent_id,  content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
     ...
     # connect with connect id
    __connectstr__ = get_object_or_404(__connectmoduletitle__, pk=__connectid__, **viewable_dict)
    object = get_object_or_404(__modelname__, __connectid__ = __connectid__, pk=content_type_id, active=True, **viewable_dict)    

    context = {
        'parent_page': '___PARENTPAGE___',
        'page': 'view___pluralmodname__',
        'ref_parent_id': ref_parent_id,  
        '__connectstr__': __connectstr__,  # '___connectstr___'
        '__connectid__': __connectid__,  # '___connectid___'
        'parent_id': parent_id,  
    ...
    template_file = f"{app_name}/{module_path}/view____pluralmodname__.html"
    return render(request, template_file, context)



@login_required
def copy___singularmodname__(request, __connectid__ , parent_id, content_type_id):
    user = request.user
    ref_parent_id = parent_id
    parent_id = None if parent_id == 0 else parent_id
    viewable_dict, first_viewable_dict = get_viewable_dicts(request.user, 
    ...
    # connect with connect id
    __connectstr__ = get_object_or_404(__connectmoduletitle__, pk=__connectid__, **viewable_dict)
    tobject = get_object_or_404(__modelname__, id=content_type_id, 
                                __connectid__ = __connectid__, 
