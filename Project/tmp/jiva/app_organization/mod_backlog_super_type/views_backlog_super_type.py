...
from app_organization.mod_backlog_super_type.forms_backlog_super_type import *
...
# ============================================================= #
@login_required
def list_backlog_super_types(request, pro_id):
    # take inputs
    # process inputs
    user = request.user       
    objects_count = 0    
    ...
    if search_query:
        ...
        tobjects = BacklogSuperType.objects.filter(active=True, pro_id=pro_id, **viewable_dict).order_by('position')
        ...
        if 'selected_item' in request.POST:  
            ...
            for item_id in selected_items:
                ...
                if bulk_operation == 'bulk_delete':
                    ...
                    return redirect('list_backlog_super_types', pro_id=pro_id)
            return redirect('list_backlog_super_types', pro_id=pro_id)
...
# ============================================================= #
@login_required
def list_deleted_backlog_super_types(request, pro_id):
    # take inputs
    # process inputs
    user = request.user        
    objects_count = 0    
        ...
        if 'selected_item' in request.POST:  
                ...
                for item_id in selected_items:
                    ...
                    if bulk_operation == 'bulk_restore':
                        ...
                        return redirect('list_deleted_backlog_super_types', pro_id)
                return redirect('list_deleted_backlog_super_types', pro_id=pro_id)
...
# Create View
@login_required
def create_backlog_super_type(request, pro_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
    if request.method == 'POST':
...
# Edit
@login_required
def edit_backlog_super_type(request, pro_id , super_type_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
...
@login_required
def delete_backlog_super_type(request, pro_id ,super_type_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
    object = get_object_or_404(BacklogSuperType, pk=super_type_id, active=True, **viewable_dict)
...
@login_required
def permanent_deletion_backlog_super_type(request, pro_id ,super_type_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
...
@login_required
def restore_backlog_super_type(request, pro_id, super_type_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
...
@login_required
def view_backlog_super_type(request,  pro_id, super_type_id):
    user = request.user
    # connect with connect id
    pro = get_object_or_404(Project, pk=pro_id, **viewable_dict)
    object = get_object_or_404(BacklogSuperType, pk=super_type_id, active=True, **viewable_dict)    
