...
def view_project_metrics_flow_tab(request, project_id):
    ...
    for snapshot_time in snapshot_points:
        if CFD_BY == "DATE":
            ...
            best_cfd_counts = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
            ...
            for card, transitions in card_transitions.items():          
                ...
                if to_state and to_state_lc in best_cfd_counts[s_date_formatted]:
                    best_cfd_counts[s_date_formatted][to_state_lc][card.id]  += 1
                ...
                for col in active_columns:
                    ...
                    if not best_cfd_counts[s_date_formatted][col_lc]:
                        best_cfd_counts[s_date_formatted][col_lc] = defaultdict(int, {0: 0})  # Add placeholder 0
            
                last_nonzero_backlog = 0

                # ✅ Initialize BACKLOG_COUNT for each time step
                for s_date_formatted in sorted(best_cfd_counts.keys()):  # Ensure sorted order
                    if 'BACKLOG_COUNT' not in best_cfd_counts[s_date_formatted]:
                        best_cfd_counts[s_date_formatted]['BACKLOG_COUNT'] = 0  # Default to 0

                    # ✅ Fill with the previous non-zero backlog value if current is 0
                    if best_cfd_counts[s_date_formatted]['BACKLOG_COUNT'] == 0:
                        best_cfd_counts[s_date_formatted]['BACKLOG_COUNT'] = last_nonzero_backlog
                    else:
                        last_nonzero_backlog = best_cfd_counts[s_date_formatted]['BACKLOG_COUNT']  # Update non-zero backlog
