...
def view_project_metrics_flow_tab(request, project_id):
    ...
    counter = 1
    output_cfd_data = defaultdict(lambda: defaultdict(int))
    ...
    first_data[BACKLOG_COLUMN_NAME] = backlog_count  # Add backlog count

    output_cfd_data[first_time] = first_data.copy()  # Store first row

    print(f">>> === {counter}: CFD:{first_time} {first_data} === <<<")
    counter += 1
    ...
    for (prev_time, _), (curr_time, curr_data) in pairwise(sorted_items):
        ...
        # Reference the previous row from output_cfd_data (cumulative values)
        prev_cumulative = output_cfd_data[prev_time]

        # Store the current row as-is
        output_cfd_data[curr_time] = curr_data.copy()

        # Create the cumulative next row
        backlog_count = 0
        for each_column in active_columns:
            each_column = each_column.lower()
            output_cfd_data[curr_time][each_column] = prev_cumulative[each_column] + curr_data[each_column]
            backlog_count += output_cfd_data[curr_time][each_column]

        # Store backlog count for the current row
        output_cfd_data[curr_time][BACKLOG_COLUMN_NAME] = backlog_count

        counter += 1

    # Print the processed output_cfd_data
    for time, data in sorted(output_cfd_data.items()):
        print(f"CFD:{time} => {data}")
