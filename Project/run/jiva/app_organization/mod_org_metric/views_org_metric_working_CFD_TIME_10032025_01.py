    ...
    for snapshot_time in snapshot_points:
        if CFD_BY == "DATE":
            ...
            for card, transitions in card_transitions.items():
                ...
                backlog_count = Backlog.objects.filter(pro=project, active=True, created_at__lte=reference_transition_time).count()
            ...
            # ✅ Calculate total cards across all columns for the specified time
            # for s_time in best_cfd_counts.keys():
            #     if 'BACKLOG_COUNT' not in best_cfd_counts[s_time]:
            #         best_cfd_counts[s_time]['BACKLOG_COUNT'] = 0  # Default value if missing
