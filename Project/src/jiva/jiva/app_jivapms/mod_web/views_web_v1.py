...
def index(request):
    ...
    if user.is_authenticated:
        ...
        logger.debug(f">>> === User is authenticated: {user.id} === <<<")
        if user.is_superuser:
            logger.debug(f">>> === User is superuser: {user} === <<<")
            super_user = True
            role = COMMON_ROLE_CONFIG["SUPER_USER"]["name"]
        else:
            logger.debug(f">>> === User is normal user: {user} === <<<")
            member = Member.objects.get(user__id=user.id)
            logger.debug(f">>> === Member is normal user: {member} === <<<")
        ...
        if user.is_anonymous:
            logger.debug(f">>> === User is anonymous: {user} === <<<")
            role = COMMON_ROLE_CONFIG["NO_ROLE"]["name"]
    ...
    logger.debug(f">>> === index-page ROLE: {role} === <<<")
    ...
    try:
        ...
        logger.error(f"Template not found using general: {template_url}")
